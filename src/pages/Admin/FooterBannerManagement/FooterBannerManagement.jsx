...
import { getFooterBanner, upsertFooterBanner } from "../../../utils/footerBannerService";
import { getImageUrl } from "../../../utils/imageUtils";

const FooterBannerManagement = () => {
  ...
  const loadFooterBanner = async () => {
    try {
      setIsLoading(true);
      setError("");
      const response = await getFooterBanner();
      ...
      console.error("Error loading footer banner:", err);
      setError(err.message || "Failed to load footer banner data");
      setFooterBanner(null);
    } finally {
      setIsLoading(false);
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await upsertFooterBanner(submitFormData);
      toast.success("Footer banner saved successfully!");
      await loadFooterBanner();
    } catch (err) {
      const errorMessage = err.message || "Failed to save footer banner";
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-gray-500 mt-1">
          Manage the footer banner displayed on the homepage
        </p>
      </div>
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-6">
            ...
            {footerBanner && (
              <div className="pt-4 border-t border-gray-200">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                  ...
                  {footerBanner.created_at && (
                    <div>
                      <label className="block text-xs font-semibold text-gray-500 mb-1">
                        Created At
                      </label>
                      <p className="text-sm text-gray-900">
                        {formatDateTime(footerBanner.created_at)}
                      </p>
                    </div>
                  )}
                  {footerBanner.updated_at && (
                    <div>
                      <label className="block text-xs font-semibold text-gray-500 mb-1">
                        Last Updated
                      </label>
                      <p className="text-sm text-gray-900">
                        {formatDateTime(footerBanner.updated_at)}
                      </p>
                    </div>
                  )}
                </div>
            ...
            <div className="flex justify-end pt-2">
              ...
              >
                ...
                {isSaving ? "Saving..." : "Save Footer Banner"}
              </button>
            </div>
          </form>
        )}
...
export default FooterBannerManagement;
