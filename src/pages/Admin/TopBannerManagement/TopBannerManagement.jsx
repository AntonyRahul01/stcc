...
import { getTopBanner, upsertTopBanner } from "../../../utils/topBannerService";
import { getImageUrl } from "../../../utils/imageUtils";

const TopBannerManagement = () => {
  ...
  const loadTopBanner = async () => {
    try {
      setIsLoading(true);
      setError("");
      const response = await getTopBanner();
      ...
      setError(err.message || "Failed to load top banner data");
      setTopBanner(null);
    } finally {
      setIsLoading(false);
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await upsertTopBanner(submitFormData);
      toast.success("Top banner saved successfully!");
      await loadTopBanner();
    } catch (err) {
      const errorMessage = err.message || "Failed to save top banner";
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-gray-500 mt-1">
          Manage the top banner displayed on the homepage
        </p>
      </div>
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-6">
            ...
            {topBanner && (
              <div className="pt-4 border-t border-gray-200">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                  ...
                  {topBanner.created_at && (
                    <div>
                      <label className="block text-xs font-semibold text-gray-500 mb-1">
                        Created At
                      </label>
                      <p className="text-sm text-gray-900">
                        {formatDateTime(topBanner.created_at)}
                      </p>
                    </div>
                  )}
                  {topBanner.updated_at && (
                    <div>
                      <label className="block text-xs font-semibold text-gray-500 mb-1">
                        Last Updated
                      </label>
                      <p className="text-sm text-gray-900">
                        {formatDateTime(topBanner.updated_at)}
                      </p>
                    </div>
                  )}
                </div>
            ...
            <div className="flex justify-end pt-2">
              ...
              >
                ...
                {isSaving ? "Saving..." : "Save Top Banner"}
              </button>
            </div>
          </form>
        )}
